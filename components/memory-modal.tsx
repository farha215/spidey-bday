  ...
  return (
    <div className="absolute inset-x-4 top-10 bottom-14 z-50 panel-in flex flex-col justify-end sm:inset-x-8 sm:bottom-20">
      <div className="bit-border bg-[#0a0a18] p-5 shadow-2xl" style={{ "--bb-step": "3px", "--bb-frame": "#96e0f7", "--bb-fill": "#0a0a18" } as any}>
        ...
        <div className="mb-4 flex aspect-video w-full items-center justify-center overflow-hidden border-2 border-[#96e0f7] bg-[#111827]">
          {m.photo ? (
            <img src={m.photo} alt={m.title} className="h-full w-full object-cover" />
          ) : (
            <div className="text-center font-pixel-body text-[6px] text-[#96e0f7] opacity-60">📷 NO PHOTO</div>
          )}
        </div>
        <h3 className="mb-2 font-pixel-body text-[9px] text-white">{m.title}</h3>
        <p className="font-pixel-body text-[7px] leading-loose text-[#96e0f7]">{m.caption}</p>
      </div>
    </div>
  );
}
